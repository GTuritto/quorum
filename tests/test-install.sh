...
test "$(sed -n '1p' "$test_root/version.out")" = "quorum-skill 0.1.66"
...
grep -F "QUORUM v0.1.66" "$test_root/all.out" >/dev/null
...
if command -v pwsh >/dev/null 2>&1; then
  ...
  grep -F "QUORUM v0.1.66" "$test_root/powershell.out" >/dev/null
else
